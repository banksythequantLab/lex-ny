...
function getConfig(): CogneeConfig | null {
...
async function getAuthHeader(): Promise<Record<string, string>> {
  const cfg = getConfig();
  if (!cfg) throw new Error("Cognee not configured");
 ...
 */
export async function remember(entry: CogneeMemoryEntry): Promise<{ ok: boolean; details?: string }> {
  const cfg = getConfig();
  if (!cfg) return { ok: false, details: "Cognee not configured" };
...
export async function recall(
  query: string,
  opts: { session_id?: string; topK?: number } = {}
  ...
  try {
    ...
    const data = (await r.json()) as { results?: CogneeRecallHit[] } | CogneeRecallHit[];
...
export async function cogneeHealthCheck(): Promise<{ ok: boolean; details: string }> {
...
export function getCogneeStats(): CogneeStats {
  const cfg = getConfig();
