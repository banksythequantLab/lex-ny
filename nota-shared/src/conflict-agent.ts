...
import { getWebDataClient, type WebDataClient } from "./bright-data.js";
type WebDataProvider = "brightdata" | "nimble";
import type { ConflictMatch, ConflictReport, FilingKind } from "./types.js";
 ...
 * retired.
 */
async function searchUSPTO(client: WebDataClient, mark: string): Promise<Partial<ConflictMatch>[]> {
  ...
  try {
    ...
    const extracted = await chatJSON<{ matches: Array<{
 ...
 */
async function searchUSCO(client: WebDataClient, mark: string): Promise<Partial<ConflictMatch>[]> {
 ...
 */
async function searchNYBusiness(client: WebDataClient, mark: string): Promise<Partial<ConflictMatch>[]> {
...
async function searchDEBusiness(client: WebDataClient, mark: string): Promise<Partial<ConflictMatch>[]> {
...
async function searchWYBusiness(client: WebDataClient, mark: string): Promise<Partial<ConflictMatch>[]> {
 ...
 */
async function searchCommonLaw(client: WebDataClient, mark: string, classDescriptions: string[]): Promise<Partial<ConflictMatch>[]> {
...
export interface RunConflictSearchOpts {
  mark: string;
  classes: number[];
  class_descriptions?: string[];   // human-readable, e.g. ["Computer software", "Education services"]
 ...
 */
export async function runConflictSearch(opts: RunConflictSearchOpts): Promise<ConflictReport> {
  ...
  const [usptoMatches, uscoMatches, nyMatches, deMatches, wyMatches, commonLawMatches] =
    await Promise.all([
...
3. For each match, a similarity score (0.0-1.0) and one-sentence reasoning`,
  ...
  const scoredMatches: ConflictMatch[] = analysis.scored_matches
  ...
  const report: ConflictReport = {
    ...
    web_data_provider: client.provider as "brightdata" | "nimble",
