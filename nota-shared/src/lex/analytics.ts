...
export interface CitedDecision {
  opinion_id: string;
  cl_id: string | null;   // CourtListener cluster id (opinions.source_id)
  case_name: string;
  court_id: string;
  decision_date: string | null;
  inbound: number;        // how many corpus opinions cite this one
}
...
function mapDecision(r: any): CitedDecision {
  return {
...
export async function mostCitedDecisions(
  opts: { limit?: number; courtId?: string } = {}
  ...
  const { rows } = await getPool().query(
    `SELECT o.id opinion_id, o.source_id cl_id, o.case_name, o.court_id, o.decision_date::text decision_date, ic.inbound
...
export async function judgeInfluenceRanking(
  opts: { limit?: number; minOpinions?: number } = {}
): Promise<JudgeInfluence[]> {
  const limit = Math.min(100, opts.limit ?? 20);
  const minOpinions = opts.minOpinions ?? 5;
  const { rows } = await getPool().query(
    `SELECT j.id judge_id, j.full_name name,
...
export async function judgeProfile(
  judgeId: string,
  opts: { topN?: number } = {}
  ...
  const meta = await p.query(
    `SELECT j.id, j.full_name, j.cl_person_id,
            count(DISTINCT oj.opinion_id)::int authored,
      ...
      WHERE j.id = $1::uuid
      GROUP BY j.id, j.full_name, j.cl_person_id`,
    [judgeId]
  );
  if (meta.rows.length === 0) return null;
  const m = meta.rows[0];
  const top = await p.query(
    `SELECT o.id opinion_id, o.source_id cl_id, o.case_name, o.court_id, o.decision_date::text decision_date,
...
export async function citedBy(
  ...
  if (seedRes.rows.length === 0) return { seed: null, citers: [], total_citers: 0 };
  const seed = mapDecision(seedRes.rows[0]);
  const citers = await p.query(
    `SELECT o.id opinion_id, o.source_id cl_id, o.case_name, o.court_id, o.decision_date::text decision_date,
            COALESCE(ic.inbound, 0) inbound
      ...
      ORDER BY COALESCE(ic.inbound, 0) DESC
      LIMIT $2`,
    [seed.opinion_id, limit]
  );
  return { seed, citers: citers.rows.map(mapDecision), total_citers: seed.inbound };
}

export interface OpinionDetail {
  opinion_id: string; cl_id: string | null; case_name: string; court_id: string;
  decision_date: string | null; citation: string | null; ai_summary: string | null;
  text: string | null; inbound: number;
}

/** Full opinion (by CourtListener cluster id) for the source viewer: metadata,
 *  body text (capped), and inbound citation count. */
export async function getOpinion(clId: string): Promise<OpinionDetail | null> {
  const r = await getPool().query(
    `SELECT o.id, o.source_id, o.case_name, o.court_id, o.decision_date::text dt,
            o.citation, o.ai_summary, left(o.text_plain, 40000) AS text,
            COALESCE(ic.inbound, 0) inbound
       FROM opinions o
       LEFT JOIN opinion_inbound_counts ic ON ic.opinion_id = o.id
      WHERE o.source_id = $1 LIMIT 1`,
    [clId]
  );
