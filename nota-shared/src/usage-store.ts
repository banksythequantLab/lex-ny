 ...
 */
export function loadFromDisk<T>(name: string, limit: number): T[] {
  ensureDir();
  ...
  try {
    ...
    console.warn(`[usage-store] loadFromDisk(${name}) failed:`, (e as Error).message);
    return [];
  }
}
 ...
 */
export function appendToDisk<T>(name: string, entry: T): void {
  ensureDir();
  const fp = filePath(name);
  try {
    ...
    console.warn(`[usage-store] appendToDisk(${name}) failed:`, (e as Error).message);
  }
}

function maybeRotate(fp: string): void {
  try {
    ...
    console.warn(`[usage-store] maybeRotate(${fp}) failed:`, (e as Error).message);
  }
}
 ...
 */
export function clearDisk(name: string): void {
  const fp = filePath(name);
  try {
    if (fs.existsSync(fp)) fs.unlinkSync(fp);
  } catch (e) {
    console.warn(`[usage-store] clearDisk(${name}) failed:`, (e as Error).message);
  }
}
