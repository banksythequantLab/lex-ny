...
export class NycAmLegalScraper {
    webClient;
    constructor(webClient = getWebDataClient()) {
    ...
    async scrapeNode(nodeId) {
        ...
        let text;
        const bodyMatch = html.match(/<div[^>]*id="document-content"[^>]*>([\s\S]*?)<\/div>\s*<\/div>/i) ||
        ...
        let level = "section";
        if (/^Title\s+\d/i.test(title))
            level = "title";
        else if (/^Chapter\s+\d/i.test(title))
            level = "chapter";
        else if (/^Subchapter/i.test(title))
            level = "subchapter";
        else if (/^Article/i.test(title))
            level = "article";
        else if (childIdSet.size > 5)
            level = "container";
        return {
            nodeId,
            title,
     ...
     */
    async crawl(startNodeId = "1", maxNodes = 50) {
 ...
 */
export function parseJustiaListing(html) {
    const out = [];
    ...
    while ((m = linkRegex.exec(html)) !== null) {
        const path = m[1];
        if (!path.match(/\/(court-of-appeals|appellate-division)/))
            continue;
...
export function parseJustiaCase(url, html) {
    ...
    const bodyMatch = html.match(/<div[^>]*class="[^"]*opinion-body[^"]*"[^>]*>([\s\S]*?)<\/div>/i) ||
...
export async function liveSerpLegalSearch(query, opts = {}) {
    ...
    const results = await client.searchSerp(augmentedQuery, {
        engine: "google",
        limit,
    });
...
function matchField(html, label) {
    ...
    const strongRe = new RegExp(`<strong[^>]*>${label}[:]?[\\s]*</strong>([^<\\n]+)`, "i");
...
//# sourceMappingURL=justia-amlegal.js.map