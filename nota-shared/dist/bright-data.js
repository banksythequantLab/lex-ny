...
class BrightDataUsageTracker {
    ...
    constructor() {
        ...
        try {
            ...
            console.warn("[bright-data] usage tracker disk load failed:", e.message);
        }
    }
    record(entry) {
...
export class BrightDataClient {
    ...
    async healthCheck() {
        ...
        try {
            ...
            if (!fetchUrl_ok)
                errors.push("BD Web Unlocker returned unexpected content");
        }
        catch (e) {
        ...
        try {
            ...
            if (!serp_ok)
                errors.push("BD SERP returned no results");
        }
        catch (e) {
 ...
 */
export function parseGoogleHtmlResults(html, limit) {
    const results = [];
    const seen = new Set();
    ...
    for (let i = 1; i < blocks.length && results.length < limit; i++) {
        ...
        let snippet = "";
        const snippetMatch = block.match(/<div[^>]*class="[^"]*VwiC3b[^"]*"[^>]*>([\s\S]{20,500}?)<\/div>/) ||
...
/*  Adapter factory                                                    */
/* ------------------------------------------------------------------ */
export function getWebDataClient() {
