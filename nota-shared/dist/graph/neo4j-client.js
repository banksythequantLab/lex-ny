...
export function getDriver() {
    ...
    const cfg = getConfig();
    if (!cfg) {
        throw new Error("Neo4j not configured. Set NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD in .env.local");
    }
 ...
 */
export async function syncOpinions(rows) {
    if (rows.length === 0)
        return 0;
 ...
 */
export async function syncStatutes(rows) {
    if (rows.length === 0)
        return 0;
    return withSession(async (s) => {
        const result = await s.run(`
 ...
 */
export async function syncOpinionCitations(rows) {
    if (rows.length === 0)
        return 0;
    return withSession(async (s) => {
        const result = await s.run(`
 ...
 */
export async function syncOpinionApplies(rows) {
    if (rows.length === 0)
        return 0;
    return withSession(async (s) => {
        const result = await s.run(`
...
export async function expandViaGraph(opts) {
    ...
    return withSession(async (s) => {
        ...
        const citing = seedOpinionClIds.length === 0
            ? []
            : (await s.run(`
          ...
          LIMIT $lim
          `, { clIds: seedOpinionClIds, lim: neo4j.int(perBucketLimit) })).records;
        const cited = seedOpinionClIds.length === 0
            ? []
            : (await s.run(`
          ...
          LIMIT $lim
          `, { clIds: seedOpinionClIds, lim: neo4j.int(perBucketLimit) })).records;
        const related = seedStatuteCitationKeys.length === 0
            ? []
            : (await s.run(`
          ...
          LIMIT $lim
          `, { keys: seedStatuteCitationKeys, lim: neo4j.int(perBucketLimit) })).records;
        return {
...
export async function getGraphStats() {
    ...
    `).catch(() => ({ records: [] }));
        ...
        for (const r of nodeCounts.records) {
            node_counts[r.get("label")] = r.get("c").toNumber();
        }
        const relationship_counts = {};
        for (const r of relCounts.records) {
            relationship_counts[r.get("relationshipType")] = r.get("c").toNumber();
        }
