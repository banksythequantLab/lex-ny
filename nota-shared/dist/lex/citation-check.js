...
function extractStatutes(text) {
    ...
    for (const m of text.matchAll(STATUTE_RE)) {
        ...
        if (lawId)
            out.push({ raw: `${m[1]} ${m[2]}`.replace(/\s+/g, " "), lawId, loc: m[2] });
    }
...
export async function checkBrief(text) {
    const p = getPool();
    const checks = [];
    const norm = text.replace(/\s+/g, " "); // collapse line wraps before extraction
    for (const s of extractStatutes(norm)) {
        const r = await p.query(`SELECT law_name, title FROM statutes WHERE law_id=$1 AND location_id=$2 AND doc_type='SECTION' LIMIT 1`, [s.lawId, s.loc]);
        if (r.rows.length) {
            checks.push({
    ...
    for (const c of extractCases(norm)) {
        const r = await p.query(`SELECT o.source_id, o.case_name, o.court_id, o.decision_date::text dt,
              similarity(o.case_name, $1) sim, COALESCE(ic.inbound, 0) inbound
         FROM opinions o
         LEFT JOIN opinion_inbound_counts ic ON ic.opinion_id = o.id
        WHERE o.case_name % $1
        ORDER BY similarity(o.case_name, $1) DESC, COALESCE(ic.inbound, 0) DESC LIMIT 1`, [c.name]);
        const row = r.rows[0];
