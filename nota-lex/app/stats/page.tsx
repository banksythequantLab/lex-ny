...
/**
 * /stats - live corpus + sponsor dashboard.
 *
 * Single screen that proves Lex.NY is real: pulls live counts from
 * /api/corpus-stats (local Postgres + Neo4j) and each sponsor stats endpoint,
 * rendering them as a wall of live counters. This is the demo-video money shot.
 */
...
export default function StatsPage() {
  ...
  useEffect(() => {
    fetch("/api/corpus-stats")
      .then((r) => r.json())
      .then(setCorpus)
      .catch((e) => setCorpus({ postgres: { ok: false, error: String(e) } as never }))
    ...
    eps.forEach((ep) => {
      fetch("/api/" + ep)
        .then((r) => r.json())
        .then((d) => setSponsors((s) => ({ ...s, [ep]: d })))
        .catch(() => setSponsors((s) => ({ ...s, [ep]: { ok: false } })));
    });
  }, []);
    ...
    <main className="min-h-screen bg-[var(--color-paper)] text-[var(--color-ink)]">
      ...
      <div className="max-w-[1180px] mx-auto px-7 py-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          <HeroStat label="Total legal records" value={pg?.total_legal_records} accent loading={loading} />
          <HeroStat label="Case decisions" value={pg?.opinions} loading={loading} />
          <HeroStat label="NY docket records" value={pg?.ny_cases} loading={loading} />
          <HeroStat label="Statute sections" value={pg?.statutes} loading={loading} />
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-14">
          <div>
            ...
            <div className="border border-[var(--color-rule)]/30 rounded-sm overflow-hidden">
              ...
              }) : <div className="px-4 py-8 text-center text-sm text-[var(--color-ink-2)]">{loading ? "Loading…" : "—"}</div>}
            </div>
          ...
          <div>
            ...
            <div className="border border-[var(--color-rule)]/30 rounded-sm p-5">
              ...
              <div className="space-y-2 mb-5">
                ...
                )) : <div className="text-sm text-[var(--color-ink-2)]">{loading ? "Loading…" : "—"}</div>}
              </div>
...
function HeroStat({ label, value, accent, loading }: { label: string; value?: number; accent?: boolean; loading?: boolean }) {
  return (
    <div className={"rounded-sm border p-5 " + (accent ? "border-[var(--color-seal-deep)] bg-[var(--color-seal)]/5" : "border-[var(--color-rule)]/30 bg-[var(--color-paper-2)]")}>
      <div className={"font-[family-name:var(--font-display)] text-4xl mb-1 tabular-nums " + (accent ? "text-[var(--color-seal-deep)]" : "")}>
        {loading && value === undefined ? <span className="text-[var(--color-ink-2)]">···</span> : fmt(value)}
      </div>
...
function MiniStat({ label, value, pending }: { label: string; value?: number; pending?: boolean }) {
  return (
    <div className="rounded-sm border border-[var(--color-rule)]/30 p-4">
      <div className="font-[family-name:var(--font-display)] text-2xl mb-0.5 tabular-nums">
        {fmt(value)}{pending ? <span className="text-xs text-[var(--color-ink-2)] ml-1">building…</span> : null}
      </div>
      <div className="font-[family-name:var(--font-mono)] text-[10px] tracking-wider uppercase text-[var(--color-ink-2)]">{label}</div>
    </div>
  );
...
function SponsorCard({ name, sub, data }: { name: string; sub: string; data: unknown }) {
  const d = data as { health?: unknown; ok?: boolean; stats?: { implementation_status?: string } } | undefined;
  let ok = false;
  if (d) {
    if (typeof d.health === "string") ok = true;
